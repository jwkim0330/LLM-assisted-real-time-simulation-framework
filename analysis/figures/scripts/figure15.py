# -*- coding: utf-8 -*-
"""
Windows용 Figure 15 재현 코드

입력:
    preprocessed_total_group/seed_metrics_tidy.csv
또는
    preprocessed_total_group/seed_metrics_tidy_total.csv
또는
    seed_metrics_tidy.csv

출력:
    Figure 15. Paired seed-level comparison of total-passenger
    performance across the three scenarios..tiff
    Figure15_preview.png
...
OUTPUT_PREVIEW_NAME = "Figure15_preview.png"
...
def locate_metrics_csv(script_dir: Path, user_path: str | None) -> Path:
    ...
    path = project_root / "data" / "processed" / "total" / "total_group_seed_metrics.csv"
    if not path.is_file():
        raise FileNotFoundError(f"입력 CSV를 찾지 못했습니다: {path}")
    return path


def load_total_metrics(csv_path: Path) -> pd.DataFrame:
    ...
    # Group 열이 있으면 Total 관련 행만 우선 선택
    if "Group" in df.columns:
        group_series = df["Group"].astype(str).str.strip().str.lower()
        total_mask = group_series.isin({"total", "all", "overall", "totalgroup", "total_group"})
        if total_mask.any():
    ...
    if df.empty:
        raise ValueError("사용 가능한 Total-group 데이터가 없습니다.")
...
def make_figure15(
    ...
    for column, scenario in enumerate(SCENARIO_ORDER):
        subset = total_df.loc[
            total_df["Scenario"].astype(str) == scenario
        ].copy()
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Windows에서 최종 Figure 15를 재현합니다."
    )
    parser.add_argument(
        "--metrics_csv",
        type=str,
        default=None,
        help="Total-group seed metrics CSV의 전체 경로",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def main() -> None:
    ...
    total_df = load_total_metrics(metrics_csv)
    make_figure15(total_df, output_tiff, output_preview)
